...
> {
  ...
  return Object.entries(grouped).map(([roundId, teams]) => {
    ...
    const ranks: number[] = sorted.map((t, i) => {
      if (i === 0) return 1;
      return sorted[i - 1].pts === t.pts ? ranks[i - 1] : i + 1;
    });
    return {
      roundId,
  ...
  for (const leagueId of Object.keys(result)) {
    ...
    result[leagueId].forEach((s, i) => {
      if (i === 0) {
        ...
        const prev = result[leagueId][i - 1];
        s.rank = prev.totalPoints === s.totalPoints ? prev.rank : i + 1;
      }
    });
  }
