...
function getBotResponse(input: string): string {
  ...
  if (q.includes("チーム") && q.includes("数"))
    return "KSL Season 1には全48チームが参加しています。Division 1〜6の6つのディビジョンに分かれ、各8チームが戦います。";
  if (q.includes("場所") || q.includes("会場"))
    return "メイン会場は鹿児島ポーカーラウンジ MEGAです。会場情報ページでアクセス方法やGoogleマップへのリンクをご確認いただけます。";
      ...
      <div className="flex-shrink-0 bg-white border-t border-slate-200 px-4 py-3">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          ...
          >
            <svg
              className="w-4 h-4"
              style={{ color: inputText.trim() ? "#be185d" : "#94a3b8" }}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2.5}
